...
import { Plus, MessageSquare, Trash2, Edit2, Menu, X, ChevronLeft, ChevronRight, Settings, ChevronDown, FlaskConical } from 'lucide-react'
...
export function Sidebar({
  ...
  const [toolsDropdownPosition, setToolsDropdownPosition] = useState<{ bottom: number; left: number } | null>(null)
  ...
  useEffect(() => {
    ...
    const handleClickOutside = (event: MouseEvent) => {
      ...
      if (toolsButtonRef.current && !toolsButtonRef.current.contains(target) &&
  ...
  const sidebarContent = (
    <>
      {/* Header */}
      <div className={`${isCollapsed ? 'p-2' : 'p-4'} border-b border-[var(--color-primary-navy)]/10 dark:border-[var(--color-white)]/5 backdrop-blur-sm transition-all duration-300`}>
        <button
          onClick={onNewChat}
            ...
            <div className="space-y-2">
              {chats.map(chat => (
                ...
                {editingChat === chat.id ? (
                  ...
                  <>
                    <h3 className={`font-semibold text-sm truncate pr-12 leading-tight ${
                      currentChatId === chat.id ? 'text-[var(--color-white)]' : 'text-[var(--color-primary-navy)] dark:text-[var(--color-white)]'
                    }`}>
                      {chat.title}
                    </h3>
                    <p className={`text-xs mt-2 truncate leading-relaxed ${
                      currentChatId === chat.id ? 'text-[var(--color-white)]/85' : 'text-[var(--color-primary-navy)]/60 dark:text-[var(--color-white)]/60'
                    }`}>
                      {chat.preview}
                    </p>
                    <p className={`text-[11px] mt-1.5 ${
                      currentChatId === chat.id ? 'text-[var(--color-white)]/70' : 'text-[var(--color-primary-navy)]/50 dark:text-[var(--color-white)]/50'
                    }`}>
                ...
                {/* Action buttons */}
                {(hoveredChat === chat.id || currentChatId === chat.id) && !editingChat && (
                  <div className="absolute right-2.5 top-3 flex gap-2">
                    <button
                      ...
                      title="Rename"
                    >
                      <Edit2 className={`h-3.5 w-3.5 transition-colors ${
                        currentChatId === chat.id
      ...
      <div className={`mt-auto border-t border-[var(--color-primary-navy)]/10 dark:border-[var(--color-white)]/5 backdrop-blur-sm overflow-visible ${isCollapsed ? 'p-2' : 'p-3'}`}>
        {!isCollapsed ? (
          <div className="space-y-2">
            {/* Tools Dropdown */}
            ...
            >
              <div className="flex items-center gap-2.5">
                ...
                <span className="text-sm font-semibold text-[var(--color-primary-navy)] dark:text-[var(--color-white)]">Tools</span>
              </div>
              <ChevronDown className={`h-4 w-4 text-[var(--color-primary-navy)]/60 dark:text-[var(--color-white)]/60 transition-all group-hover:text-[var(--color-primary-navy)] dark:group-hover:text-[var(--color-white)] ${isToolsOpen ? 'rotate-180' : ''}`} />
            </button>
            ...
            {isToolsOpen && selectedAgentId && (
              <div className="absolute bottom-full left-0 right-0 mb-2 bg-[var(--color-background)] rounded-lg shadow-lg border border-[var(--color-border)] max-h-80 overflow-y-auto z-20">
                <div className="py-2">
                  {agents.find(a => a.id === selectedAgentId)?.tools.map((tool, index) => (
                    <a
                      key={index}
                      href={tool.url}
                      target="_blank"
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <span className="font-medium text-sm block">{tool.display_name}</span>
            ...
            >
              ...
              <span className="text-sm font-semibold text-[var(--color-primary-navy)] dark:text-[var(--color-white)]">MLFlow Monitoring</span>
            </button>
          </div>
        ) : (
          <div className="space-y-2">
  ...
  return (
    <>
      ...
      {mounted && isToolsOpen && selectedAgentId && toolsDropdownPosition && createPortal(
        <div
        ...
        >
          <div className="px-4 py-2 border-b border-[var(--color-primary-navy)]/20 dark:border-[var(--color-white)]/20">
            <h3 className="font-medium text-sm text-[var(--color-primary-navy)] dark:text-[var(--color-white)]">Tools & Resources</h3>
          </div>
          <div className="py-2">
            {agents.find(a => a.id === selectedAgentId)?.tools.map((tool, index) => (
              <a
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <span className="font-medium text-sm block text-[var(--color-primary-navy)] dark:text-[var(--color-white)]">{tool.display_name}</span>
