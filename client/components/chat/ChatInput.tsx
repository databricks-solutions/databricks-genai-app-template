...
export function ChatInput({ onSendMessage, disabled = false, selectedAgentId: propSelectedAgentId, onAgentChange, hasMessages = false }: ChatInputProps) {
  ...
  const handleInput = (e: React.ChangeEvent<HTMLTextAreaElement>) => {
    ...
    if (textarea) {
      ...
      const newHeight = Math.min(textarea.scrollHeight, parseFloat(getComputedStyle(document.documentElement).getPropertyValue('--chat-input-max-height')))
          ...
          <div className="flex items-end gap-3 p-4">
            ...
            <div className="flex items-center gap-2 flex-shrink-0">
              ...
              <div className="relative agent-dropdown-container">
                <button
                  onClick={() => !hasMessages && setIsAgentDropdownOpen(!isAgentDropdownOpen)}
                  ...
                  title={hasMessages ? 'Agent locked for this chat' : 'Select agent'}
                >
                  <span className="text-xs font-medium text-[var(--color-foreground)]">
                    {currentAgent?.display_name || 'Agent'}
                  </span>
                  {!hasMessages && (
                    <ChevronDown className={`h-3 w-3 text-[var(--color-muted-foreground)] transition-transform ${isAgentDropdownOpen ? 'rotate-180' : ''}`} />
                  )}
                </button>
                  ...
                  {agents.map((agent) => (
                    <button
                      ...
                      className={`w-full text-left px-4 py-3 hover:bg-[var(--color-muted)] transition-colors flex items-start justify-between gap-3 ${
                        selectedAgent === agent.id ? 'bg-[var(--color-muted)]' : ''
                      }`}
                    >
