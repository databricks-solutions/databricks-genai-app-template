...
export function ChartRenderer({ visualization }: ChartRendererProps) {
  ...
  const bgColor = getComputedStyle(document.documentElement).getPropertyValue('--color-bg-elevated').trim()
  ...
  const enhancedData = type === 'table' ? data : {
    ...data,
    datasets: 'datasets' in data ? data.datasets?.map((dataset, index) => ({
      ...dataset,
      borderColor: dataset.borderColor || getDefaultColor(index),
      backgroundColor: dataset.backgroundColor || (type === 'line' 
...
function getDefaultColor(index: number): string {
  const colors = [
    getComputedStyle(document.documentElement).getPropertyValue('--color-chart-primary').trim(),
    getComputedStyle(document.documentElement).getPropertyValue('--color-chart-secondary').trim(),
    getComputedStyle(document.documentElement).getPropertyValue('--color-chart-accent').trim()
  ]
  // Fallback colors if CSS variables are not set
  const fallbacks = ['#22D3EE', '#06B6D4', '#10B981']

  const color = colors[index % colors.length] || fallbacks[index % fallbacks.length]
  return color
}
      ...
      <table className="min-w-full divide-y divide-[var(--color-border)]">
        ...
        <tbody className="divide-y divide-[var(--color-border)]">
          {data.rows.map((row, rowIndex) => (
            <tr key={rowIndex} className="hover:bg-[var(--color-bg-elevated)] transition-all duration-200">
