...
export function CustomThemeProvider({ children }: ThemeProviderProps) {
  ...
  const applyColorsToCSS = (newColors: ThemeColors) => {
    ...
    const accentSecondary = adjustBrightness(newColors.accentPrimary, isLightTheme ? 10 : -15)
    ...
    const accentParticles = animRgb ? `rgba(${animRgb.r}, ${animRgb.g}, ${animRgb.b}, 0.7)` : newColors.animatedBgColor
    ...
    const chatText = isLightColor(newColors.accentPrimary) ? '#1B3139' : '#FFFFFF'
    ...
    if (scrollRgb) {
      root.style.setProperty('--color-scrollbar-track', `rgba(${scrollRgb.r}, ${scrollRgb.g}, ${scrollRgb.b}, 0.05)`)
      root.style.setProperty('--color-scrollbar-thumb', `rgba(${scrollRgb.r}, ${scrollRgb.g}, ${scrollRgb.b}, 0.2)`)
      root.style.setProperty('--color-scrollbar-thumb-hover', `rgba(${scrollRgb.r}, ${scrollRgb.g}, ${scrollRgb.b}, 0.3)`)
    }
  ...
  const updateAnimatedBackground = (newSettings: Partial<AnimatedBackgroundSettings>) => {
    ...
    localStorage.setItem('themeAnimatedBackground', JSON.stringify(updatedSettings))
  }
